...
jQuery(function($) {
	...
	$.fn.nyroModal.settings = {
		...
		linkTo: false, // Whether to link to the file or not
		...
		titleContainer: '<div class="clearfix" id="nyroModalTitle"><a href="#" class="nyroModalClose " id="closeBut" title="close">Close</a><h1></h1></div>',
	...
	function processModal(settings) {
		...
		if (from) {
			...
			if (currentSettings.type == 'form') {
				...
				var title = jFrom.attr('alt') || currentSettings.defaultImgAlt;
				currentSettings.title = currentSettings.title || title;
				initModal();
	...
	// Set the modal Title
	function setTitle() {
		var title = $('#nyroModalTitle').find('h1');
		if (title.length>0)
		{
			title.text(currentSettings.title);
		} else
		{
			title = $(currentSettings.titleContainer);
			title.find('h1').html(currentSettings.title);
			modal.contentWrapper.prepend(title);
		}
	}
	...
	function fillContent() {
		...
		if (!currentSettings.modal)
			modal.wrapper.prepend(currentSettings.titleContainer);
	...
	function showBackground(elts, settings, callback) {
		elts.bg.css({opacity:0}).fadeTo(500, 0.50, callback);
	}
